...
HRESULT CInputJoypad::Init(HINSTANCE hInst, HWND hWnd)
{
	...
	for (int iCnt = 0; iCnt < KEY_MAX; iCnt++)
	{
		m_aKeyState[(KEY)iCnt] = false;
	}
	...
	{
#ifdef _DEBUG
		MessageBox(hWnd, "ジョイパッドが接続されていません", "警告！", MB_ICONWARNING);
#endif	
		return hr;
	}
...
HRESULT CInputJoypad::Update(void)
{
	...
	if (!m_pDIDevice || !m_bEnable)
	{
		for (int iCnt = 0; iCnt < KEY_MAX; iCnt++)
		{
			m_aKeyState[(KEY)iCnt] = false;
		}
		m_JoyStickL = Vector2(0.0f, 0.0f);
		m_JoyStickR = Vector2(0.0f, 0.0f);
...
{
	//dualshock4
	m_JoyStickL = Vector2((float)m_joypadState.lX, (float)-m_joypadState.lY);
	m_JoyStickR = Vector2((float)m_joypadState.lZ, (float)-m_joypadState.lRz);

	//□
	if (m_joypadState.rgbButtons[0])
	{
		m_aKeyState[KEY_X] = true;
	}
	else
	{
		m_aKeyState[KEY_X] = false;
	}

	//×
	if (m_joypadState.rgbButtons[1])
	{
		m_aKeyState[KEY_Y] = true;
	}
	else
	{
		m_aKeyState[KEY_Y] = false;
	}

	//○
	if (m_joypadState.rgbButtons[2])
	{
		m_aKeyState[KEY_A] = true;
	}
	else
	{
		m_aKeyState[KEY_A] = false;
	}

	//△
	if (m_joypadState.rgbButtons[3])
	{
		m_aKeyState[KEY_B] = true;
	}
	else
	{
		m_aKeyState[KEY_B] = false;
	}

	//L1
	if (m_joypadState.rgbButtons[4])
	{
		m_aKeyState[KEY_LB] = true;
	}
	else
	{
		m_aKeyState[KEY_LB] = false;
	}

	//R1
	if (m_joypadState.rgbButtons[5])
	{
		m_aKeyState[KEY_RB] = true;
	}
	else
	{
		m_aKeyState[KEY_RB] = false;
	}

	//L2
	if (m_joypadState.rgbButtons[6])
	{
		m_aKeyState[KEY_LT] = true;
	}
	else
	{
		m_aKeyState[KEY_LT] = false;
	}

	//R2
	if (m_joypadState.rgbButtons[7])
	{
		m_aKeyState[KEY_RT] = true;
	}
	else
	{
		m_aKeyState[KEY_RT] = false;
	}

	//Share
	if (m_joypadState.rgbButtons[8])
	{
		m_aKeyState[KEY_L_PUSH] = true;
	}
	else
	{
		m_aKeyState[KEY_L_PUSH] = false;
	}

	//Options
	if (m_joypadState.rgbButtons[9])
	{
		m_aKeyState[KEY_R_PUSH] = true;
	}
	else
	{
		m_aKeyState[KEY_R_PUSH] = false;
	}
	...
	{
		m_aKeyState[KEY_BACK] = true;
	}
	else
	{
		m_aKeyState[KEY_BACK] = false;
	}
	...
	{
		m_aKeyState[KEY_GUIDE] = true;
	}
	else
	{
		m_aKeyState[KEY_GUIDE] = false;
	}
