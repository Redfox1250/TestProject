...
//CScene2D生成
//------------------------------------------------
CScene2D *CScene2D::Create(Vector3 pos, Vector3 rot, Vector2 polygonSize, const char *pFileName)
{
	CScene2D *pScene2D;

	pScene2D = new CScene2D;
	pScene2D->Init(pos, rot, polygonSize, pFileName);
...
//CScene2Dコンストラクタ
//------------------------------------------------
CScene2D::CScene2D(int nPriority, CScene::OBJECT_TYPE objType, CScene::SCENE_TYPE SceneType) : CScene(nPriority, objType, SceneType)
{
	m_pTexture = NULL;
	m_pVtxBuff = NULL;
...
//初期化処理
//------------------------------------------------
HRESULT CScene2D::Init(Vector3 pos, Vector3 rot, Vector2 polygonSize, const char *pFileName)
{
...
//画面サイズと同じ大きさで初期化
//------------------------------------------------
void CScene2D::InitBack(const char *pFileName)
{
	Init(Vector3(0.0f, 0.0f, 0.0f), Vector3(0.0f, 0.0f, 0.0f), Vector2(0.0f, 0.0f), pFileName);
	SetVertexPos(Vector3(0.0f, 0.0f, 0.0f),
		Vector3(SCREEN_WIDTH, 0.0f, 0.0f),
		Vector3(0.0f, SCREEN_HEIGHT, 0.0f),
...
//頂点座標の設定
//------------------------------------------------
void CScene2D::SetVertexPos(Vector3 LeftUp, Vector3 RightUp, Vector3 LeftDown, Vector3 RightDown)
{
	VERTEX_2D *pVtx;
	...
	pVtx[0].pos = m_pos + LeftUp;
	pVtx[1].pos = m_pos + RightUp;
	pVtx[2].pos = m_pos + LeftDown;
	pVtx[3].pos = m_pos + RightDown;
...
void CScene2D::SetVertexTex(void)
{
	...
	pVtx[0].tex = Vector2(m_posTex.x, m_posTex.y);
	pVtx[1].tex = Vector2(m_posTex.x + 1.0f, m_posTex.y);
	pVtx[2].tex = Vector2(m_posTex.x, m_posTex.y + 1.0f);
	pVtx[3].tex = Vector2(m_posTex.x + 1.0f, m_posTex.y + 1.0f);
...
//テクスチャ座標の設定
//------------------------------------------------
void CScene2D::SetVertexTex(Vector2 tex0, Vector2 tex1, Vector2 tex2, Vector2 tex3)
{
	VERTEX_2D *pVtx;
...
//位置座標の設定
//------------------------------------------------
void CScene2D::SetPosition(Vector3 pos)
{
	...
	pVtx[0].pos += pos - m_pos;
	pVtx[1].pos += pos - m_pos;
	pVtx[2].pos += pos - m_pos;
	pVtx[3].pos += pos - m_pos;
...
//位置座標の加算
//------------------------------------------------
void CScene2D::AddPostion(Vector3 pos)
{
	m_pos += pos;
	VERTEX_2D *pVtx;
...
void CScene2D::SetVertexAlpha(float fAlpha)
{
	if (fAlpha >= 1.0f)
	{
		fAlpha = 1.0f;
...
void CScene2D::SetSize(Vector2 vSize)
{
	m_fLength = SetLength(vSize.x, vSize.y);
	m_fAngle = SetAngle(vSize.x, vSize.y);
